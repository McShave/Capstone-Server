...
def start_background_tasks():
    ...
    # Start BLE clients
    bme_process = Process(target=run_asyncio_coroutine, args=(bme_main(),))  # No need to pass sensor_db
    water_process = Process(target=run_asyncio_coroutine, args=(water_main(),))  # No need to pass sensor_db
    smoke_process = Process(target=run_asyncio_coroutine, args=(smoke_main(),))  # No need to pass sensor_db
...
def create_app():
    ...
    @app.route('/')
    def home():
        # Fetch fresh sensor data from the database
        sensor_data = sensor_db.get_all_sensor_data()  # Fetch all sensor data
